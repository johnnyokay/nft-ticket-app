...
export const UserProvider = ({ children }: Props, { chains }: any) => {
    ...
    const resolveEns = async (provider: ethers.providers.Web3Provider, address: string) => {
        let ens;
        try {
    ...
    const updateUser = async (provider: ethers.providers.Web3Provider) => {
        ...
        if (ens) {
            ...
            displayName = address.substring(0,6) + "..." + address.substring(address.length-4, address.length);
        }
